...
class Line:
    ...
    def process_message(self, message):
        ...
        if message.topic().lower() == 'stations': # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif message.topic().lower() == 'arrival': # Set the conditional to the arrival topic
